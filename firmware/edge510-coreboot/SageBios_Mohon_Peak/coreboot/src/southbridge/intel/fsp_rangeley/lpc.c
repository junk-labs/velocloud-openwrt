...
static int rangeley_get_pci_irq_pins(device_t dev, device_t *parent_bdg);

static int rangeley_swizzle_irq_pins(device_t dev, device_t *parent_bridge);
...
static void soc_enable_apic(struct device *dev)
{
	...
	*ioapic_index = 0;
	...
	printk(BIOS_DEBUG, "Dumping IOAPIC registers\n");
	for (i=0; i<3; i++) {
		*ioapic_index = i;
		printk(BIOS_DEBUG, "  reg 0x%04x:", i);
		reg32 = *ioapic_data;
...
static int rangeley_swizzle_irq_pins(device_t dev, device_t *parent_bridge)
{
	...
	for (parent = child->bus->dev; parent; parent = parent->bus->dev) {
		...
		/*** RANGELEY SPECIFIC ***/
		/* RCC-VE platform specific */
		/* For root port 2, Rangely has another level of routing, 
 		 * this is especially important for legacy devices that use ioapic,
 		 * devices that use MSI are not affected.		 */
		/* This also applies to port 3 and 4, but there is no pluggable legacy device
 		 * on port 3 and 4, hence they are not handled here */	


		if(PCI_SLOT(parent_devfn) == 2)
		{
			swizzled_pin ++ ;
			printk(BIOS_DEBUG, "Rangeley swizzle for slot 2 0x%02x\n",swizzled_pin );
		}
		/*** RANGELEY_SPECIFIC ***/

		printk(BIOS_DEBUG, "\tWith INT_PIN swizzled to %s\n"
			"\tAttached to bridge device, child slot %01X:%02Xh.%02Xh %02X\n",
			pin_to_str(swizzled_pin + 1), parent_bus,
			PCI_SLOT(parent_devfn), PCI_FUNC(parent_devfn),PCI_SLOT(child_devfn)  );
...
static int rangeley_get_pci_irq_pins(device_t dev, device_t *parent_bdg)
{
	...
	printk(BIOS_DEBUG, "PCI IRQ: Found device %01X:%02X.%02X using %s\n",
	...
	if (bus) {
		...
		target_pin = rangeley_swizzle_irq_pins(dev, parent_bdg);
...
static void write_pci_config_irqs(void)
{
	...
	for(irq_dev = all_devices; irq_dev; irq_dev = irq_dev->next) {
		 ...
		 */
		targ_dev = NULL;
		targ_pin = rangeley_get_pci_irq_pins(irq_dev, &targ_dev);
		...
		printk(BIOS_DEBUG, " \tINT_PIN\t\t: %d (%s)\n",int_pin, pin_to_str(int_pin));
		if (targ_devfn != devfn)
			printk(BIOS_DEBUG, "\tSwizzled to\t: %d (%s)\n",
							targ_pin, pin_to_str(targ_pin));
		printk(BIOS_DEBUG, "\tPIRQ\t\t: %c\n"
						"\tINT_LINE\t: 0x%X (IRQ %d)\n",
...
static void soc_pirq_init(device_t dev)
{
	...
	for (devn = 0; devn < NUM_IR_DEVS; devn++) {
		...
		printk(BIOS_DEBUG, " %d: ", devn);
		for (i = 0; i < 4; i++) {
			pirq = (ir->pcidev[devn] >> (i * 4)) & 0xF;
			printk(BIOS_DEBUG, "\t%-4c (%d)", 'A' + pirq, ir->pic[pirq]);
		}
		printk(BIOS_DEBUG, "\n");
	}
	...
	printk(BIOS_DEBUG, "Finished writing IRQ assignments\n");
...
static void soc_disable_smm_only_flashing(struct device *dev)
{
	...
	printk(BIOS_DEBUG, "Enabling BIOS updates outside of SMM... ");
