...
#include "../mainboard/intel/mohonpeak/gpio.h"
...
static boot_state_t bs_write_tables(void *arg)
{
	...
	#if 1
	/* enable clock for usb3 */

	{
	   int status=-1;;

	    printk(BIOS_DEBUG, "Enabling USB3 clock..\n");
	    status =  smbus_enable_usb3_clock();
		printk(BIOS_DEBUG, "status = %d\n", status);
	    
	printk(BIOS_DEBUG, "Enabling Spread Spectrum..\n");
	    status =  smbus_enable_spread_spectrum();
		printk(BIOS_DEBUG, "status = %d\n", status);
...
static boot_state_t bs_payload_boot(void *arg)
{
	struct payload *payload = arg;
       ...
       /* flip LED before handing over to payload */
        setup_soc_gpios(&gpio_map1);
