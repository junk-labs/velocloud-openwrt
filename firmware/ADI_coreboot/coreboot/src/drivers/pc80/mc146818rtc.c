...
void rtc_update_cmos_date(u8 has_century)
{
	...
	printk(BIOS_NOTICE, "RTC: Date set to %02x/%02x/%02x%02x day of week=%x\n",
...
void rtc_init(int invalid)
{
	...
	date_invalid = rtc_check_date(RTC_HAS_ALTCENTURY);
	...
	if (invalid || cmos_invalid || checksum_invalid || date_invalid) {
		printk(BIOS_WARNING, "RTC:%s%s%s%s%s\n",
		...
		if (invalid || cmos_invalid || date_invalid)
...
int rtc_check_date(u8 has_century)
{
	...
	if ( test_bcd_encoding( century )
	  ...
	  || test_bcd_encoding( dayofweek ) ) {
		printk(BIOS_NOTICE, "RTC: Invalid BCD date value\n");
		rtc_error = 1;
	}

	/* Check to see that the date values are in the correct range */
	if ( (bcd_to_hex(century) != bcd_to_hex(0x20) )
	  || (bcd_to_hex(year) > 99 )
	  || (bcd_to_hex(month) > 12 )
	  || (bcd_to_hex(dayofmonth) > 31 )
	  || (bcd_to_hex(dayofweek) > 7 ) ) {
		printk(BIOS_NOTICE, "RTC: Invalid date range\n");
		rtc_error = 1;
	}
	return rtc_error;
}
