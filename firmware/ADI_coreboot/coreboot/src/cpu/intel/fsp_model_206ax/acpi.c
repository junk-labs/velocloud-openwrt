...
static int generate_P_state_entries(int core, int cores_per_package)
{
	...
	int coord_type, power_max, power_unit;
	int num_entries, total_entries, free_slots;
	...
	free_slots = PSS_MAX_ENTRIES;
	total_entries = 0;

	/* reserve an entry for the max non-turbo ratio */
	free_slots--;
    total_entries++;

	if(get_turbo_state() == TURBO_ENABLED) {
		/* reserve a p-state entry for the turbo p-state */
		free_slots--;
		total_entries++;
	}
	...
	/*  calculate the minimum ratio step that will generate a number of p-states
		that will fit in the available slots */
	while (num_entries > free_slots) {
		ratio_step <<= 1;
		num_entries >>= 1;
	}
	...
	len_pss = acpigen_write_package(total_entries);
