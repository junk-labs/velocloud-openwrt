...
struct bev_s {
    int type;
    u32 vector;
    int harddrive_index;  /* Hard drive index, support boot from multiple hard disks */
};
static struct bev_s BEV[20];
static int BEVCount;
static int HaveFDBoot;
static int HardDrive_num = 0; /* global variable to keep track of total number hard disk */

static void
add_bev(int type, u32 vector)
{
     /*	printf("ww-debug, add_bev(), type=%d, BEVCount=%d\n",type, BEVCount);*/
	...
	bev->harddrive_index = HardDrive_num;
	HardDrive_num ++; 
...
boot_disk(u8 bootdrv, int checksig)
{
	...
	u32 mbr_sig;
	mbr_sig = GET_FARVAR(bootseg, mbr->signature);
...
do_boot(int seq_nr)
{
	...
	printf("Booting from Floppy...\n");
        ...
        break;
    case IPL_TYPE_HARDDISK:
       	my_bootdrv = 0x80 + BEV[seq_nr].harddrive_index ;
       	boot_disk(my_bootdrv, 1);
...
handle_18(void)
{
    ...
    dprintf(2, "Boot failure recovery - BootSequence=%d seq=%d\n",BootSequence, seq);
    BootSequence = seq;
    do_boot(seq);
}
