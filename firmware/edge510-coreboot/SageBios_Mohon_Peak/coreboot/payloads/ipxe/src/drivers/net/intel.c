...
/* External M88E1514 PHY */
#define PHY_REVISION_MASK 0xFFFFFFF0
#define M88E1514_E_PHY_ID	0x01410DD0
...
static int read_phy_reg_mdic(struct intel_nic *intel,uint32_t offset, uint16_t *data );

static void read_phy_id(struct intel_nic *intel, uint32_t *phy_id);

static int write_phy_reg_mdic(struct intel_nic *intel,uint32_t offset, uint16_t data );

static int marvel_88e151x_init(struct intel_nic *intel) ;
...
static int intel_reset ( struct intel_nic *intel ) {
	...
	/*For Rangely, check external PHY, M88E1514 only on port 0 and 1 */
	if(intel->port < 2)
	{
		read_phy_id(intel,&phy_id);
		if(phy_id == M88E1514_E_PHY_ID) 
		{
			marvel_88e151x_init(intel);
		}
...
static struct pci_device_id intel_nics[] = {
	...
	PCI_ROM ( 0x8086, 0x1f40, "i354-SD", "I354 SERDES", 0 ),
	PCI_ROM ( 0x8086, 0x1f41, "i354-ext", "I354 Ext Phy", 0 ),
	PCI_ROM ( 0x8086, 0x1f45, "i354-2.5", "I354 2.5GBps", 0 ),
...
#define MAX_PHY_REG_ADDRSS 0xFF
...
static int read_phy_reg_mdic(struct intel_nic *intel,uint32_t offset, uint16_t *data )
{
	...
	if(offset >  MAX_PHY_REG_ADDRSS)
	{
	       	printf ("PHY Address %d out of range\n", offset);	
	 	ret =  -1;
		goto out;
	}
	...
	writel ( mdic, intel->regs + INTEL_MDIC );
...
static int write_phy_reg_mdic(struct intel_nic *intel,uint32_t offset, uint16_t data )
{
	...
	if(intel->port >=2)
	{
		printf("Only port 0 and 1 has PHY\n");
		ret = 0;
		goto out;
	}
	...
	mdic = (((uint32_t)data) |
		...
		(0x0 << E1000_MDIC_PHY_SHIFT) |  /* legacy*/
		(E1000_MDIC_OP_WRITE));
	...
	if (!(mdic & E1000_MDIC_READY)) {
		printf( "MDI Write did not complete offset=%d, data=0x%x\n",
			 offset, data);
		ret = -1;
		goto out;
	}
...
#define RETURN_IF_FAIL(_ret) do { if(_ret!= 0) return _ret;} while(0)
...
static int marvel_88e151x_init(struct intel_nic *intel) 
{
	...
	ret= write_phy_reg_mdic(intel,22, 0x00ff ); /* page register*/
	RETURN_IF_FAIL(ret);

	ret= write_phy_reg_mdic(intel,17, 0x214b );
	RETURN_IF_FAIL(ret);

	ret= write_phy_reg_mdic(intel,16, 0x2144 );
	RETURN_IF_FAIL(ret);

	ret= write_phy_reg_mdic(intel,17, 0x0c28 );
	RETURN_IF_FAIL(ret);

	ret= write_phy_reg_mdic(intel,16, 0x2146 );
	RETURN_IF_FAIL(ret);

	ret= write_phy_reg_mdic(intel,17, 0xb233 );
	RETURN_IF_FAIL(ret);

	ret= write_phy_reg_mdic(intel,16, 0x214d );
	RETURN_IF_FAIL(ret);

	ret= write_phy_reg_mdic(intel,17, 0xcc0c );
	RETURN_IF_FAIL(ret);

	ret= write_phy_reg_mdic(intel,16, 0x2159 );
	RETURN_IF_FAIL(ret);


	ret= write_phy_reg_mdic(intel,22, 0x0000 );
	RETURN_IF_FAIL(ret);

        /* for 88E1514 set to operate in SGMII-to-Copper mode */
	/* reg 20 [2:0] set to 1 */
	ret= write_phy_reg_mdic(intel,22, 0x0012 ); /* page register*/
	RETURN_IF_FAIL(ret);
	ret = read_phy_reg_mdic(intel, 20, &reg);
	RETURN_IF_FAIL(ret);
	reg = (reg & 0xfff8 ) | 0x1;
	ret= write_phy_reg_mdic(intel,20, reg );
	RETURN_IF_FAIL(ret);

	/* soft reset after setting mode, assert bit 15 of reg 20 */
	ret = read_phy_reg_mdic(intel, 20, &reg);
	RETURN_IF_FAIL(ret);
	reg = (reg & 0x7fff ) | 0x8000;
	ret= write_phy_reg_mdic(intel,20, reg );
	RETURN_IF_FAIL(ret);
 	
	/* set page register back to page 0 */
	ret= write_phy_reg_mdic(intel,22, 0x0000 );
	RETURN_IF_FAIL(ret);
...
static void read_phy_id(struct intel_nic *intel, uint32_t *phy_id)
{
	...
	ret = write_phy_reg_mdic(intel,22, 0x0000 ); /* page register*/
	if(ret !=0 )
	{
		printf("Error: exiting ID read\n");
		return;
	}

	ret = read_phy_reg_mdic(intel, 2, &id1);
	if(ret !=0 )
	{
		printf("Error: exiting ID read\n");
		return;
	}
	my_id = (uint32_t) (id1 << 16);
		
	ret =  read_phy_reg_mdic(intel, 3, &id2);
	if(ret !=0 )
	{
		printf("Error: exiting ID read\n");
		return;
	...
	my_id |= (uint32_t)(id2 & PHY_REVISION_MASK);

	(*phy_id) = my_id;
	
	// printf("ww-debug PHY ID1=0x%x, ID2-0x%x ID=0x%x\n", (uint32_t)id1, (uint32_t)id2, *phy_id);
