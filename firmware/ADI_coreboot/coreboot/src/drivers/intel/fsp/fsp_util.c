...
#if ! IS_ENABLED(CONFIG_FSP_USES_SHARED_STRUCT)
...
#else /* #if ! IS_ENABLED(CONFIG_FSP_USES_SHARED_STRUCT) */
...
)
{
	...
	// MLR todo
	// ChipsetFspReturnPoint_SharedData (Status, HobListPtr, SharedData);
	ChipsetFspReturnPoint (Status , HobListPtr);
...
)
{
	...
	FspInfoHeader = (FSP_INFO_HEADER *)find_fsp();
	...
	if (FspVpdRgn->PcdVpdRegionSign != VPD_IMAGE_ID)
		die("Error: FSP VPD Region signature does not match.");

	if (FspVpdRgn->PcdImageRevision != VPD_IMAGE_REV)
		die("Error: FSP VPD Region version does not match.");

	/* Copy default data from Flash */
	memcpy (FspUpdRgn, (void *)(FspInfoHeader->ImageBase +
		FspVpdRgn->PcdUpdRegionOffset), sizeof(UPD_DATA_REGION));

	/* Verify the UPD data region is valid */
	if (FspUpdRgn->PcdRegionTerminator != 0x55AA)
		die("Error: FSP UPD Region terminator is not valid.");

	FspInitParams.RtBufferPtr = (FSP_INIT_RT_BUFFER *)&FspRtBuffer;
	FspInitParams.ContinuationFunc = (CONTINUATION_PROC)AAsmContinuation;
	...
	/* Call the chipset code to fill in the chipset specific structures */
	chipset_fsp_early_init(&FspInitParams, FspInfoHeader, &SharedData);

	/* Call back to romstage for board specific changes */
	romstage_fsp_rt_buffer_callback(&FspRtBuffer);


	printk(BIOS_DEBUG, "Calling into FSP\n");
...
static void fsp_after_pci_enum(void *unused)
{
	 ...
	 * This call needs to be done after bus enumeration but before
	 * resource allocation.
	 */
...
BOOT_STATE_INIT_ENTRIES(finalize_fsp) = {
	BOOT_STATE_INIT_ENTRY(BS_PAYLOAD_BOOT, BS_ON_ENTRY,
			fsp_finalize, NULL),
};

BOOT_STATE_INIT_ENTRIES(after_pci_enum_fsp) = {
	BOOT_STATE_INIT_ENTRY(BS_DEV_ENUMERATE, BS_ON_EXIT,
			fsp_after_pci_enum, NULL),
};
