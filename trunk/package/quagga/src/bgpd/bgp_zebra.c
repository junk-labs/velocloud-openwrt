...
zebra_read_ipv4 (int command, struct zclient *zclient, zebra_size_t length)
{
  ...
  struct redist_aspath redist_aspath;
  ...
  if (!bgp)
    ...
    memset(&redist_aspath, 0, sizeof(struct redist_aspath));
  ...
  if (command == ZEBRA_IPV4_ROUTE_ADD)
      ...
      if (BGP_DEBUG(zebra, ZEBRA))
	{
	  ...
	  zlog_debug("Zebra rcvd: IPv4 route add %s %s/%d nexthop %s metric %u tag %d, asl=%d, instance %s",
      ...
      if (BGP_DEBUG(zebra, ZEBRA))
	{
	  ...
	  zlog_debug("Zebra rcvd: IPv4 route delete %s %s/%d "
		     "nexthop %s metric %u tag %d instance %s",
		     zebra_route_string(api.type),
		     inet_ntop(AF_INET, &p.prefix, buf[0], sizeof(buf[0])),
		     p.prefixlen,
		     inet_ntop(AF_INET, &nexthop, buf[1], sizeof(buf[1])),
...
bgp_zebra_announce (struct prefix *p, struct bgp_info *info, struct bgp *bgp, safi_t safi)
{
  ...
  memset(&bgp_attr_buf, 0, sizeof(struct zapi_bgp_attr)); 
  ...
  if (p->family == AF_INET)
	   ...
	   mpinfo = bgp_info_mpath_next (mpinfo))
	{
      ...
      if (peer->bgp->name) {
         namelen = strnlen(peer->bgp->name, INSTANCE_NAMSIZ);
         strncpy (api.iname, peer->bgp->name, namelen);
         api.iname[namelen] = '\0';
      }
      else
         api.iname[0] = '\0';
      ...
      if (distance)
	{
      ...
      bgp_attr_buf.aspath_val[0] = bgp->as; // Add own AS
      SET_FLAG (api.message, ZAPI_MESSAGE_ASPATH);
      ...
      if (BGP_DEBUG(zebra, ZEBRA))
	{
	  int i;
	  char buf[2][INET_ADDRSTRLEN];
	  zlog_debug("Zebra send: IPv4 route add %s/%d nexthop %s metric %u tag %d"
		     " count %d iname: %s",
		     inet_ntop(AF_INET, &p->u.prefix4, buf[0], sizeof(buf[0])),
		     p->prefixlen,
		     inet_ntop(AF_INET, api.nexthop[0], buf[1], sizeof(buf[1])),
		     api.metric, api.tag, api.nexthop_num, api.iname);
  ...
  if (p->family == AF_INET6)
	...
	{
	  char buf[2][INET6_ADDRSTRLEN];
	  zlog_debug("Zebra send: IPv6 route add %s/%d nexthop %s metric %u tag %d",
  ...
  if (p->family == AF_INET)
    {
      ...
      bgp_attr_buf.aspath_len = aspath_count_hops(info->attr->aspath) + 1;
      bgp_attr_buf.aspath_val[0] = peer->bgp->as; // Add own AS
      SET_FLAG (api.message, ZAPI_MESSAGE_ASPATH);
      if (info->attr->aspath->segments) {
          int i = 1;
          struct assegment *seg = info->attr->aspath->segments;
	...
	{
	  char buf[2][INET_ADDRSTRLEN];
	  zlog_debug("Zebra send: IPv4 route delete %s/%d nexthop %s metric %u tag %d iname %s",
		     inet_ntop(AF_INET, &p->u.prefix4, buf[0], sizeof(buf[0])),
		     p->prefixlen,
		     inet_ntop(AF_INET, nexthop, buf[1], sizeof(buf[1])),
		     api.metric, api.tag, api.iname);
