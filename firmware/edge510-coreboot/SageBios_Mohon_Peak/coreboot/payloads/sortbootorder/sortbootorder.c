...
#define FLASH_BASE 0xFF800000 /* 8M flash  base address */
...
static char bootOrder_buff[0x1000];

static int flash_address, flash_offset;
...
int main(void) {
	...
	flash_address = (int)tmp - FLASH_BASE; 
	flash_offset = flash_address;
	printf("flash_address = 0x%x\n", flash_address);
	if ((u32)tmp & 0xfff)
	...
	// printf("bootlist_def_ln=%d bootlist_map_ln=%d \n",bootlist_def_ln, bootlist_map_ln);	
...
static int strcmp_printable_char(const char *s1, const char *s2)
{
	...
	/* printf("strcmp_printable_char, s1=%s, s2=%s\n",s1,s2);*/
	for (i = 0; 1; i++) {
		res = s1[i] - s2[i];
		if (s2[i] == '\0')
		{
		/* 	printf("strcmp_printable_char i=%d, break1\n",i);*/
			break;
		}
		...
		{
		/*	printf("strcmp_printable_char i=%d, break2\n",i); */
			break;
		}
		#endif 
	}
...
static void save_flash(char buffer[MAX_DEVICES][MAX_LENGTH], u8 max_lines) {
	...
	if (!flash)
		...
		/* read the entire 4K out first */
		addr = flash_offset; //0x700000  /*flash_address*/;

		printf("read flash flash_address=0x%x, size=%d\n",addr, FLASH_SIZE_CHUNK);
		
		ret = flash->read(flash,addr, FLASH_SIZE_CHUNK, bootOrder_buff );
		printf("read flash flash_address=0x%x, ret=%d\n",addr, ret);

		flash_offset = flash_address;

		/* update the buffer with the new formatted list */
		printf("Updating bootorder buffer..count = %d\n", count);
		for(i=0; i< count; i++)
		{
			bootOrder_buff[i] = cbfs_formatted_list[i];
		}

		addr =  flash_offset; //0x700000 /*flash_address*/;

		printf("Erasing Flash size 0x%x @ 0x%x\n", FLASH_SIZE_CHUNK, addr);
		flash->erase(flash, addr, FLASH_SIZE_CHUNK);
		flash->spi->rw = SPI_WRITE_FLAG;

		addr = flash_offset; // 0x700000 /*flash_address */;
		printf("writing the entire 4K back \n");
		flash->write(flash,addr, FLASH_SIZE_CHUNK, bootOrder_buff);
#if 0
