...
{
	ROMSTAGE_CONST struct device *usb_dev;
	u32 usb_devfn;
	pci_devfn_t usb_devfn_t, usb_hcdid_devfn_t;
	u32 class;
	int err = 0;
	...
	if (CONFIG_USBDEBUG_HCD_INDEX > 0) {
		usb_hcdid_devfn_t = pci_ehci_dbg_dev(CONFIG_USBDEBUG_HCD_INDEX);
	...
	pci_devfn_t dev = usb_devfn_t;
#else
	device_t dev = usb_dev;
#endif
		...
		if (ehci_debug_hw_enable(usb_devfn_t, &ehci_base, &dbg_offset))
			return -1;

		/* Initialize the USB debug registers and console */
		if (usbdebug_init_(ehci_base, dbg_offset, dbg_info) >= 0) {
