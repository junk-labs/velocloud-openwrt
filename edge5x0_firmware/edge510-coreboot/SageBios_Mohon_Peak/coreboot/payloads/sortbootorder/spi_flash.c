			...
			u32 offset, size_t len)
{
	...
	/* offset = 0x730000;*/
	 printf("SF: Erasing ..., offset=0x%x len=%d sector_size=%d\n", offset, len, erase_size);
	...
	while (offset < end) {
		...
		if (ret)
			goto out;
		printf("SF: CMD_WRITE_ENABLE done\n");

		ret = spi_flash_cmd_write(flash->spi, cmd, sizeof(cmd), NULL, 0);
		printf("spi_flash_cmd_write ret=%x\n",ret);;

		if (ret)
			goto out;

		printf("SF: CMD_WRITE done\n");
	...
	// printf("SF: Successfully erased %u bytes @ %#x\n", len, start);
