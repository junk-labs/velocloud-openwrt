			   ...
			   u32 addr, size_t len, const void *buf)
{
	...
	if (addr + len > flash->size) {
		printk (BIOS_ERR,
			"ERROR: Attempt to write 0x%x-0x%x which is out of chip\n",
...
static uint8_t chipset_uses_4_bit_spi_rom_size(void)
{
	uint8_t uses_4_bit = 0;
#if IS_ENABLED(CONFIG_SOUTHBRIDGE_INTEL_FSP_LYNXPOINT)
	uses_4_bit = 1;
#elif IS_ENABLED(CONFIG_SOUTHBRIDGE_INTEL_LYNXPOINT)
	uses_4_bit = 1;
#endif
	return uses_4_bit;
}
...
static struct spi_flash *spi_flash_hwseq(struct spi_slave *spi)
{
	...
	if (chipset_uses_4_bit_spi_rom_size()) {
		flash->size = 1 << (19 + (flcomp & 0x0f));
		if ((cntlr.hsfs & HSFS_FDV) && ((cntlr.flmap0 >> 8) & 3))
			flash->size += 1 << (19 + ((flcomp >> 4) & 0x0f));
	} else {
		flash->size = 1 << (19 + (flcomp & 7));
		if ((cntlr.hsfs & HSFS_FDV) && ((cntlr.flmap0 >> 8) & 3))
			flash->size += 1 << (19 + ((flcomp >> 3) & 7));
	}
	printk (BIOS_DEBUG, "flash size 0x%lx bytes\n", (unsigned long)flash->size);
