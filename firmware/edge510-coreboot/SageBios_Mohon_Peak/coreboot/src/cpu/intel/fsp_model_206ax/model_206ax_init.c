...
{
#ifndef CONFIG_ENABLE_VMX
#error "CONFIG_ENABLE_VMX must be set."
#endif
...
static void intel_cores_init(device_t cpu)
{
	...
	printk(BIOS_DEBUG, "Hyperthreading is disabled.\n");
#else
	...
	conf = cpu->chip_info;
	/* Find the maximum C-State used */
	max_cstate = MAX(conf->c1_acpower, max_cstate);
	max_cstate = MAX(conf->c2_acpower, max_cstate);
	max_cstate = MAX(conf->c3_acpower, max_cstate);

	max_cstate = MAX(conf->c1_battery, max_cstate);
	max_cstate = MAX(conf->c2_battery, max_cstate);
	max_cstate = MAX(conf->c3_battery, max_cstate);

	msr = rdmsr(MSR_PMG_CST_CONFIG_CONTROL);
	if(msr.lo & (1 << 15)) {
		printk(BIOS_ERR, "C-States already locked\n");
	} else {
		//Set the maximum C-State
		msr.lo &= ~0x7;
		msr.lo |= (0x7 & max_cstate);
...
#if IS_ENABLED(CONFIG_USE_COREBOOT_MTRR_SETUP)
	/* Setup MTRRs based on physical address size */
	struct cpuid_result cpuid_regs = cpuid(0x80000008);
...
#if IS_ENABLED(CONFIG_DISABLE_TURBO)
	/* Disable Turbo */
	disable_turbo();
#else
	/* Enable Turbo */
